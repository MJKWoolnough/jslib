...
const noop = () => {},
	...
	try {
      ...
      noTG = () => true,
      noEndpoint = params => new RPCError(0, "unknown endpoint", params);
...
export class RPC {
	...
	#connInit(conn) {
		(this.#c = conn ?? new Queue(msg => this.#c?.send(msg))).when(this.#sFn ??= ({data}) => {
			const {id, result, error, method, params} = JSON.parse(data);
			...
			if (method) {
				...
				const mid = typeof id === "string" ? parseInt(id) : id,
				      i = +!!error,
				      m = error ? new RPCError(error.code, error.message, error.data) : result;

 ...
 */
tags = <NS extends string>(ns: NS, xml: Document = document) => new Proxy({}, {"get": (_, element: string) => wrapElem(element, () => xml.createElementNS(ns, element))}) as NS extends "http://www.w3.org/1999/xhtml" ? {[K in keyof HTMLElementTagNameMap]: DOMBind<HTMLElementTagNameMap[K]>} : NS extends "http://www.w3.org/2000/svg" ? {[K in keyof SVGElementTagNameMap]: DOMBind<SVGElementTagNameMap[K]>} : NS extends "http://www.w3.org/1998/Math/MathML" ? {[K in keyof MathMLElementTagNameMap]: DOMBind<MathMLElementTagNameMap[K]>} : Record<string, DOMBind<Element>>,
/**

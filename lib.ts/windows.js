...
const snapTo = (shell, w, x3, y3) => {
    const snap = parseInt(shell.getAttribute("snap") || "0"), { offsetLeft: x1, offsetTop: y1, offsetWidth: width, offsetHeight: height } = w, x2 = x1 + width, y2 = y1 + height, x4 = x3 + width, y4 = y3 + height, mv = [0, 0];
    if (snap > 0) {
        ...
        Array.from(shell.childNodes).filter(e => e instanceof WindowElement && e !== w).forEach(e => {
            const x5 = e.offsetLeft, y5 = e.offsetTop, x6 = x5 + e.offsetWidth, y6 = y5 + e.offsetHeight;
            if (y3 <= y6 && y4 >= y5) {
    ...
    const originalLeft = this.offsetLeft, originalTop = this.offsetTop, originalWidth = this.offsetWidth, originalHeight = this.offsetHeight, grabX = e.clientX, grabY = e.clientY, mouseMove = (e) => {
    ...
    const grabX = e.clientX - this.offsetLeft, grabY = e.clientY - this.offsetTop, mouseMove = (e) => {
        const x = e.clientX - grabX, y = e.clientY - grabY, [mx, my] = snapTo(shell, this, x, y);
    ...
    shell.addEventListener("mousemove", mouseMove);
    shell.addEventListener("mouseup", mouseUp);
}, childWindows = new Map(), childOf = new Map(), alertFn = (parent, title, message, icon) => new Promise((resolve, reject) => {
    const w = windows({
        "window-hide": true,
    ...
    const ok = button({ "onclick": () => {
            resolve(data.value);
            w.remove();
        } }, "Ok"), data = autoFocus(input({ "value": defaultValue || "", "onkeydown": (e) => {
            switch (e.key) {
                case "Enter":
                    ok.click();
                    break;
...
`),
    ...
    disconnectedCallback() {
        ...
        const p = childOf.get(this), c = childWindows.get(this);
        if (p) {
    ...
    focus() {
        ...
        if (this.parentNode && this.nextElementSibling) {
            focusingWindow = this;
            const scrolls = getScrolls(this), { scrollTop, scrollLeft } = this.#slot;
...
export const shell = (props, children) => createHTML(new ShellElement(), props, children), desktop = (props, children) => createHTML(new DesktopElement(), props, children), windows = (props, children) => createHTML(new WindowElement(), props, children), noIcon = `data:image/svg+xml,%3Csvg viewBox="0 0 14 18" xmlns="${svgNS}"%3E%3Cpath d="M9,1 h-8 v16 h12 v-12 Z v4 h4" stroke="black" fill="none" /%3E%3Cpath d="M3,8 h8 m-8,3 h8 m-8,3 h8" stroke="gray" /%3E%3C/svg%3E`;

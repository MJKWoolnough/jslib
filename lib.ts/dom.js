...
export const createElements = (namespace) => (element, properties, children) => {
    ...
    return elem;
}, htmlNS = "http://www.w3.org/1999/xhtml", svgNS = "http://www.w3.org/2000/svg", createHTML = createElements(htmlNS), createSVG = createElements(svgNS), createDocumentFragment = (children) => {
    ...
    const df = document.createDocumentFragment(), fn = wrapper instanceof Function ? wrapper : document.createTextNode.bind(document);
    text.split("\n").forEach((text, n) => {
        if (n > 0) {
